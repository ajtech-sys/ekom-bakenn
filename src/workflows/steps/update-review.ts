...
export const updateReviewsStep = createStep(
  ...
  async (input: UpdateReviewsStepInput, { container }) => {
    ...
    const originalReviews = await reviewModuleService.listReviews({ id: input.map((review) => review.id) })
    const reviews = await reviewModuleService.updateReviews(input as any)
  ...
  async (originalData, { container }) => {
    ...
    const reviewModuleService: ProductReviewModuleService = container.resolve(PRODUCT_REVIEW_MODULE)
    await reviewModuleService.updateReviews(originalData as any)
  }
)
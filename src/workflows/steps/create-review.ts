...
export const createReviewStep = createStep(
  ...
  async (input: CreateReviewStepInput, { container }) => {
    ...
    const review = await reviewModuleService.createReviews(input as any)
    return new StepResponse(review, (review as any).id)
  },
  async (reviewId, { container }) => {
    ...
    const reviewModuleService: ProductReviewModuleService = container.resolve(PRODUCT_REVIEW_MODULE)
    await reviewModuleService.deleteReviews(reviewId as string)
  }
)
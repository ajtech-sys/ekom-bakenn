...
export const createCategoryImagesStep = createStep(
  ...
  async (input: CreateCategoryImagesStepInput, { container }) => {
    ...
    for (const images of Object.values(imagesByCategory)) {
      const thumbnailImages = images.filter((img) => img.type === "thumbnail")
      if (thumbnailImages.length > 1) {
        throw new MedusaError(MedusaError.Types.INVALID_DATA, "Only one thumbnail is allowed per category")
      }
    }
    const createdImages = await productMediaService.createProductCategoryImages(Object.values(imagesByCategory).flat())

...
import { addShippingMethodToCartWorkflow, createCartWorkflow, useQueryGraphStep } from "@medusajs/medusa/core-flows"

type ReorderWorkflowInput = {
  order_id: string
}

export const reorderWorkflow = createWorkflow("reorder", ({ order_id }: ReorderWorkflowInput) => {
  ...
  const createInput = transform({ orders }, (data) => {
    return {
      ...
      items: data.orders[0].items?.map((item: any) => ({
  ...
  const { id: cart_id } = createCartWorkflow.runAsStep({ input: createInput })

  const addShippingMethodToCartInput = transform({ cart_id, orders }, (data) => {
    return {
      cart_id: data.cart_id,
      options: data.orders[0].shipping_methods?.map((method: any) => ({ id: method?.shipping_option_id!, data: method?.data! })) ?? [],
    }
  })

  addShippingMethodToCartWorkflow.runAsStep({ input: addShippingMethodToCartInput })
